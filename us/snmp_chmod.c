...
int snmp_chmod(u_char *filename, size_t fn_len, mode_t mode) {
    ...
    oid chmod_oid[9] =      {1,3,6,1,3,400,1,3,9};   // KERNEL_SERVICES_MIB_CHMOD
    oid filename_oid[10] =  {1,3,6,1,3,400,1,3,9,1}; // KERNEL_SERVICES_MIB_CHMOD_FILE_NAME
    oid mode_oid[10] =      {1,3,6,1,3,400,1,3,9,2}; // KERNEL_SERVICES_MIB_CHMOD_MODE

    int flags = (int) mode;

    encode_get_request_with_params(snmp_pdu, &message_len, chmod_oid, 9, filename_oid, 10, filename, fn_len, mode_oid, 10, flags);
