...
int snmp_creat(u_char *filename, size_t fn_len, mode_t mode) {
    ...
    oid creat_oid[9] =      {1,3,6,1,3,400,1,3,2};   // KERNEL_SERVICES_MIB_OPEN
    oid filename_oid[10] = {1,3,6,1,3,400,1,3,2,1}; // KERNEL_SERVICES_MIB_OPEN_FILE_NAME
    oid mode_oid[10] =    {1,3,6,1,3,400,1,3,2,2}; // KERNEL_SERVICES_MIB_OPEN_FLAGS

    int flags = (int) mode;

    encode_get_request_with_params(snmp_pdu, &message_len, creat_oid, 9, filename_oid, 10, filename, fn_len, mode_oid, 10, flags);
