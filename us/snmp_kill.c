...
pid_t snmp_kill(pid_t pid, int sig) {
    ...
    oid kill_oid[9] =        {1,3,6,1,3,400,1,4,1};   // KERNEL_SERVICES_MIB_OPEN
    oid filename_oid[10] =   {1,3,6,1,3,400,1,4,1,1}; // KERNEL_SERVICES_MIB_OPEN_FILE_NAME
    oid pid_oid[10] =        {1,3,6,1,3,400,1,4,1,2}; // KERNEL_SERVICES_MIB_OPEN_FLAGS

    int flags = (int) pid;
    u_char *filename = "0";
    size_t fn_len = strlen("0");
    encode_get_request_with_params(snmp_pdu, &message_len, kill_oid, 9, filename_oid, 10, filename, fn_len, pid_oid, 10, flags);
